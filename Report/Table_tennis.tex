...
%\begin{itemize}
%	\item In the first iteration, set the region of interest (ROI) for the current frame %for \textit{m} times the ball diameter. Where in the subsequent frames \textit{m} is %dynamically defined according to if the ball previously.
%	\item if multiple candidate balls are produced in \textit{Pass 1}, the threshold is increased by \textit{u}\% for the next iteration and continue \textit{Pass 1}.
%	\item if no \textit{candidate balls} are produced in \textit{Pass 1}, the threshold is decreased by \textit{u}\% for the next iteration and continue \textit{Pass 1}.
%	\item if only one object remains, keep the threshold and go to \textit{Pass 2}.
%	\item If the maximum \textit{Pass 1} iterations is reached, and no suitable threshold is found, select the threshold that produced the minimum number of \textit{candidate balls} \textit{and go to \textit{Pass 2}.
%	\item cal are difference... .
%\end{itemize}}
...
\Else
\Comment{In case the position was not found previously}
	\State Adaptive Control ROI
\EndIf
\State Blurr ROI
\State First Pass (Coarse Threshold)
\If {$Candidate Balls$ found}
